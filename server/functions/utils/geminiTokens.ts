...
const TPM_LIMIT = 1_000_000;   // Tokens Per Minute limit
const TPD_LIMIT = 10_000_000; // Tokens Per Day limit

const RPM_LIMIT = 15;   // Requests Per Minute
const RPD_LIMIT = 1000; // Requests Per Day

export function trackGeminiTokens(tokensUsed: number) {
  const now = Date.now();
  ...
  if (tokenUsageThisMinute > TPM_LIMIT * 0.9) {
    console.warn("⚠️ Gemini: Approaching TPM limit!");
  }
  if (tokenUsageToday > TPD_LIMIT * 0.9) {
    console.warn("⚠️ Gemini: Approaching TPD limit!");
  }
  if (requestCountThisMinute > RPM_LIMIT * 0.9) {
    console.warn("⚠️ Gemini: Approaching RPM limit!");
  }
  if (requestCountToday > RPD_LIMIT * 0.9) {
    console.warn("⚠️ Gemini: Approaching RPD limit!");
  }
